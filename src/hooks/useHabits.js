...
export const useHabits = () => {
  ...
  const [stats, setStats] = useState(
    initialCache?.stats || { completed: 0, total: 0 }
  );
  const [phrase, setPhrase] = useState(
    initialCache?.phrase || { text: "", emoji: "" }
  );
    ...
    async (showLoading = false, force = false) => {
      ...
      try {
        ...
        const normalizedStats = data?.stats || {
          completed: normalizedHabits.filter(
            (h) => h.today_status === "completed"
          ).length,
  ...
  const markHabit = useCallback(
    async (habitId, status = "completed", date) => {
      try {
        vibrate();
        ...
        if (date === today) {
          ...
          setStats((prev) => {
            ...
            const newCompleted =
              status === "completed" ? currentHabit + 1 : currentHabit;
    ...
    async (habitId, date) => {
      try {
        ...
        if (date === today) {
          ...
          // Для других дат - НЕ трогаем todayHabits
          console.log(
            `✅ Unmarking for ${date} (not today), skipping UI update`
          );
        }
